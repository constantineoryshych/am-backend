...
export class CampaignsService extends ARestService {
	protected resultFieldContainer: string = "campaigns";

	public getAll(): Promise<unknown> {
		const method = this._DAO.getAll({ populate: [{ path: "zones" }, { path: "content" }, { path: "avatar" }] });
		return this.decorateResult(method);
	}

	public getById(id: string): Promise<unknown> {
		const method = this._DAO.getById(id, { populate: [{ path: "zones" }, { path: "content" }, { path: "avatar" }] });
		return this.decorateResult(method);
	}
